(function () {
	...
	*/
	...
	function getBlockId$1(block)
	{
		return block & 0xff;
	}

	function getBlockSlope$1(block)
	{
		return block >> 8 & 0xf;
	}
	...
	function getDataCache(x, y, z)
	{
		return dataCacheMatrix$1[x + 1 + 3 * (y + 1) + 9 * (z + 1)];
	}
	...
	function computeFaces()
	{
		...
		for(let z = 0, i = 0; z < CHUNK_WIDTH; z++) {
			for(let y = 0; y < CHUNK_WIDTH; y++) {
				for(let x = 0; x < CHUNK_WIDTH; x++, i++) {
					let block = dataCache[i];
					let id    = getBlockId$1(block);
					let slope = getBlockSlope$1(block);
